...
def evaluate_level(model: PersonalVAD, level: int, batches: int, batch_size: int, seed: int, device: torch.device) -> torch.Tensor:
    ...
    for batch in loader:
        ...
        dominant = labels[:, :frame_count].argmax(dim=-1) if labels.dim() == 3 and labels.size(-1) == 3 else labels[:, :frame_count]
        for actual in range(3):
            for predicted_label in range(3):
                confusion[actual, predicted_label] += ((dominant == actual) & (predicted == predicted_label)).sum().cpu()
    return confusion
