...
class PVADMixtureDataset(Dataset):
    ...
    def _add(self, mix: Tensor, labels: Tensor, audio: Tensor, label: int | None, start: int, gain: float) -> None:
    ...
    def _add_overlap_tracks(self, mix: Tensor, labels: Tensor, target_audio: Tensor, ntss_audio: Tensor, target_quota: int, ntss_quota: int, level: int) -> None:
        ...
        for start, end in target_intervals:
            length = end - start
            self._add(mix, labels, target_audio[target_offset:target_offset + length], None, start, self.rng.uniform(-10, -5))
            target_mask[start:end] = True
            target_offset += length
        for start, end in ntss_intervals:
            length = end - start
            self._add(mix, labels, ntss_audio[ntss_offset:ntss_offset + length], None, start, self.rng.uniform(5, 12))
            ntss_mask[start:end] = True
            ntss_offset += length
        frame_target, frame_ntss = target_mask[::160], ntss_mask[::160]
        frame_labels = labels[::160]
        frame_labels[frame_ntss] = NTSS
        frame_labels[frame_target] = TSS
    ...
    def __getitem__(self, index: int) -> dict[str, Tensor]:
        ...
        mix = torch.zeros(self.total_samples); labels = torch.full((self.total_samples,), NS, dtype=torch.long)
        ...
        if level == 1:
            ...
            for index, (audio, label, length) in enumerate(turns):
                ...
                self._add(mix, labels, audio[offset:offset + length], label, cursor, self.rng.uniform(-8, 4))
                if label == TSS:
                    target_offset += length
                else:
                    ntss_offset += length
            ...
            self._add_overlap_tracks(mix, labels, target_audio, ntss_audio, target_quota, ntss_quota, level)
        ramp = torch.linspace(self.rng.uniform(.1, 1), self.rng.uniform(1, 2), self.total_samples)
        return {"stream": (mix*ramp).clamp(-1,1), "enrollment": enrollment, "labels": labels[::160]}
