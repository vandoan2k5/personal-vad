...
from model.components import WeightedPairwiseLoss
...
def evaluate(model: PersonalVAD, device: torch.device, criterion: WeightedPairwiseLoss, global_step: int) -> dict[int, dict[str, float]]:
    ...
    for level in (1, 2, 3):
        ...
        for batch in loader:
            ...
            logits = model(stream, target_embedding)["logits"]
            frame_count = min(logits.size(1), labels.size(1))
            logits, labels = logits[:, :frame_count], labels[:, :frame_count]
...
def main() -> None:
    ...
    criterion = WeightedPairwiseLoss(HPARAMS.loss.tss_ntss_weight, HPARAMS.loss.ns_ntss_weight).to(device)
    ...
    for local_step, batch in enumerate(progress, start=1):
        ...
        optimizer.step()
        step_accuracy = accuracy(logits.detach(), labels)
        ...
        if global_step % HPARAMS.train.log_every_steps == 0:
            logger.info("iter=%d loss=%.5f accuracy=%.4f", global_step, loss.item(), step_accuracy)
